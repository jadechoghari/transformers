...
from transformers import DINOv2ExpConfig
...
if is_torch_available():
    ...
    from transformers import DINOv2ExpForImageClassification, DINOv2ExpModel
    # removed backbone
...
class Dinov2ExpModelTester:
    ...
    def create_and_check_model(self, config, pixel_values, labels):
        model = DINOv2ExpModel(config=config)
    ...
    def create_and_check_for_image_classification(self, config, pixel_values, labels):
        ...
        model = DINOv2ExpForImageClassification(config)
        model.to(torch_device)
        model.eval()
...
class Dinov2ExpModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    ...
    pipeline_model_mapping = (
        {"image-feature-extraction": DINOv2ExpModel, "image-classification": DINOv2ExpForImageClassification}
        if is_torch_available()
        else {}
    )
    fx_compatible = True
    ...
    def setUp(self):
        self.model_tester = Dinov2ExpModelTester(self)
        self.config_tester = ConfigTester(self, config_class=DINOv2ExpConfig, has_text_modality=False, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        ...
        model = DINOv2ExpModel.from_pretrained(model_name)
...
class Dinov2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = DINOv2ExpModel.from_pretrained("facebook/dinov2-base").to(torch_device)
